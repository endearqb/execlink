...
menu(mode=\"multiple\" where=!window.is_taskbar title=\"Pin/Unpin\" image=icon.pin)
{
}

menu(mode=\"multiple\" where=!window.is_taskbar title=title.more_options image=icon.more_options)
{
}
...
";
const REQUIRED_IMPORT_PLACEHOLDERS: [(&str, &str); 2] = [
    (
        "theme.nss",
        "// placeholder generated by ExecLink: keep shell import chain valid\n",
    ),
    (
        "images.nss",
        "// placeholder generated by ExecLink: keep shell import chain valid\n",
...
const IMPORT_PLACEHOLDER_MARKER: &str = "// placeholder generated by ExecLink: keep shell import chain valid";
const TASKBAR_NSS_FALLBACK: &str = r#"menu(type="taskbar" vis=key.shift() pos=0 image=\uE249)
{
...
menu(where=@(this.count == 0) type='taskbar' vis=key.shift() image=icon.settings expanded=true)
{
...
pub fn render_ai_clis_nss(config: &AppConfig) -> String {
    ...
    format!(
        "menu(type='dir|drive|back' mode='single' title='{menu_title}')\n{{\n{rendered_items}\n}}\n"
    )
}
...
fn write_shell_nss(config_root: &Path, show_nilesoft_default_menus: bool) -> AppResult<()> {
    ...
    ensure_required_import_placeholders(config_root)?;
...
fn ensure_required_import_placeholders(config_root: &Path) -> AppResult<()> {
    let imports = config_root.join("imports");
    fs::create_dir_all(&imports).map_err(|e| format!("创建 imports 目录失败: {e}"))?;

    for (file_name, placeholder) in REQUIRED_IMPORT_PLACEHOLDERS {
        let file = imports.join(file_name);
        if file.exists() {
            continue;
        }
        fs::write(&file, placeholder).map_err(|e| {
            format!(
                "写入 imports 占位文件失败 ({}): {e}",
                file.display()
            )
    ...
    ensure_taskbar_import_file(&imports)?;

    Ok(())
}

fn ensure_taskbar_import_file(imports_dir: &Path) -> AppResult<()> {
    let taskbar_file = imports_dir.join("taskbar.nss");
    if !taskbar_file.exists() {
        fs::write(&taskbar_file, TASKBAR_NSS_FALLBACK).map_err(|e| {
            format!(
                "写入 taskbar.nss 回退内容失败 ({}): {e}",
                taskbar_file.display()
            )
        })?;
        logging::log_line(&format!(
            "[config] taskbar_recovered_from_missing path={}",
            taskbar_file.display()
        ));
        return Ok(());
    }

    let current = fs::read_to_string(&taskbar_file).unwrap_or_default();
    if current.trim().is_empty() || current.contains(IMPORT_PLACEHOLDER_MARKER) {
        fs::write(&taskbar_file, TASKBAR_NSS_FALLBACK).map_err(|e| {
            format!(
                "修复 taskbar.nss 回退内容失败 ({}): {e}",
                taskbar_file.display()
            )
        })?;
        logging::log_line(&format!(
            "[config] taskbar_upgraded_from_placeholder path={}",
            taskbar_file.display()
        ));
    } else {
        logging::log_line(&format!(
            "[config] taskbar_preserved_custom path={}",
            taskbar_file.display()
        ));
    }
...
mod tests {
    ...
    #[test]
    fn should_not_mix_back_and_back_dir_in_top_level_menu_type() {
        let output = render_ai_clis_nss(&sample_config());
        assert!(output.contains("menu(type='dir|drive|back'"));
        assert!(!output.contains("back|back.dir"));
    }
    ...
    #[test]
    fn should_create_required_import_placeholders_when_missing() {
        let root = unique_temp("ai_cli_switch_placeholders");
        fs::create_dir_all(&root).unwrap();
        ...
        assert!(imports.join("theme.nss").exists());
        assert!(imports.join("images.nss").exists());
        assert!(imports.join("taskbar.nss").exists());
        let taskbar = fs::read_to_string(imports.join("taskbar.nss")).unwrap();
        assert!(taskbar.contains("menu(type=\"taskbar\""));
        assert!(taskbar.contains("menu(where=@(this.count == 0) type='taskbar'"));

        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn should_not_overwrite_existing_required_import_placeholders() {
        let root = unique_temp("ai_cli_switch_placeholders_keep");
        let imports = root.join("imports");
        fs::create_dir_all(&imports).unwrap();
        fs::write(imports.join("theme.nss"), "custom-theme-content\n").unwrap();

        let cfg = sample_config();
        apply_config(&root, &cfg).unwrap();

        let theme = fs::read_to_string(imports.join("theme.nss")).unwrap();
        assert_eq!(theme, "custom-theme-content\n");

        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn should_upgrade_placeholder_taskbar_import_to_fallback() {
        let root = unique_temp("ai_cli_switch_taskbar_upgrade");
        let imports = root.join("imports");
        fs::create_dir_all(&imports).unwrap();
        fs::write(
            imports.join("taskbar.nss"),
            "// placeholder generated by ExecLink: keep shell import chain valid\n",
        )
        .unwrap();
        ...
        assert!(taskbar.contains("menu(type=\"taskbar\""));
        assert!(taskbar.contains("menu(where=@(this.count == 0) type='taskbar'"));
        assert!(!taskbar.contains("placeholder generated by ExecLink"));

        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn should_keep_existing_taskbar_import_if_not_placeholder() {
        let root = unique_temp("ai_cli_switch_taskbar_keep");
        let imports = root.join("imports");
        fs::create_dir_all(&imports).unwrap();
        fs::write(
            imports.join("taskbar.nss"),
            "menu(type=\"taskbar\" title='custom')\n{\n}\n",
        )
        .unwrap();
        ...
        let taskbar = fs::read_to_string(imports.join("taskbar.nss")).unwrap();
        assert_eq!(taskbar, "menu(type=\"taskbar\" title='custom')\n{\n}\n");

        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn should_limit_taskbar_fallback_visibility_to_shift() {
        assert!(TASKBAR_NSS_FALLBACK.contains("menu(type=\"taskbar\" vis=key.shift()"));
        assert!(TASKBAR_NSS_FALLBACK.contains("type='taskbar' vis=key.shift()"));
        assert!(!TASKBAR_NSS_FALLBACK.contains("title=app.name"));
        assert!(!TASKBAR_NSS_FALLBACK.contains("key.lbutton()"));
    }

    #[test]
    fn should_exclude_minimal_multiple_groups_from_taskbar() {
        assert!(SHELL_NSS_MINIMAL.contains(
            "menu(mode=\"multiple\" where=!window.is_taskbar title=\"Pin/Unpin\""
        ));
        assert!(SHELL_NSS_MINIMAL.contains(
            "menu(mode=\"multiple\" where=!window.is_taskbar title=title.more_options"
        ));
    }

    #[test]
    fn should_exclude_normal_taskbar_context_from_shell_handling() {
        let rule = "exclude.where = !process.is_explorer || (window.is_taskbar && !key.shift())";
        assert!(SHELL_NSS_MINIMAL.contains(rule));
        assert!(SHELL_NSS_WITH_DEFAULTS.contains(rule));
    }
