...
- Explorer 刷新与 legacy 菜单迁移：支持扫描、迁移、清理与 Explorer 兜底刷新
...
- Windows 11 上请在 `显示更多选项` 中查看 ExecLink 菜单；当前版本不进入 Win11 顶层新右键菜单。
...
- 应用内 `高级维护` 可执行：
  - 扫描已安装的 ExecLink v2 菜单与 legacy 残留
  - 迁移旧版 PowerShell HKCU 菜单到 v2 schema
  - 通知 Explorer 刷新，或执行 Explorer 兜底重启
  - 清理旧 Nilesoft 目录与 ExecLink 菜单残留
