...
## 执行记录
- 首次执行 `npm run tauri -- build` 被 `npm run sync-version:check` 阻断，提示 `src-tauri/tauri.conf.json` 与 `package.json` 不一致。
- 按项目既定流程执行 `npm run sync-version`，重新规范化版本清单后再次构建。
- 二次执行 `npm run tauri -- build` 成功，版本校验通过：`0.2.9`。
- 产物已生成：
- `src-tauri/target/release/bundle/msi/ExecLink_0.2.9_x64_zh-CN.msi`（3493888 bytes，2026-03-06 21:35:50）
- `src-tauri/target/release/bundle/nsis/ExecLink_0.2.9_x64-setup.exe`（2319962 bytes，2026-03-06 21:35:57）

## 回顾
- 构建入口依赖 `sync-version:check`，即使版本号数值相同，只要清单文件格式发生漂移也会阻断打包；继续以 `package.json` 为单一版本源，并通过 `sync-version` 修正即可。

# 2026-03-06 菜单页顶部开关与配置页折叠重排

## 计划清单
- [x] 更新菜单页顶部栏：将菜单总开关移动到 tabs 右侧的“右键菜单分组名称”输入框后，并补充动态 hover 文案。
- [x] 将 `menu` tab 展示名从“菜单”改为“配置”，保持内部 key 不变。
- [x] 重组配置页结构：移除“启用右键菜单”卡片和“高级维护”外层容器，改为 4 个默认折叠的一级卡片并按指定顺序排列。
- [x] 同步更新使用说明文案，移除对“高级维护”的旧引用。
- [x] 删除废弃的 `ToggleRow` 组件并运行 `npm run build` 验证。

## 执行记录
- `src/pages/Home.tsx` 直接引入 `Switch`，将 `menu` tab 展示名改为“配置”，并把顶部区域调整为 `Tabs + 右键菜单分组名称输入框 + 菜单总开关`。
- 顶部菜单总开关继续复用 `config.enable_context_menu` 作为唯一状态源，并补充 hover / focus 提示气泡：关闭状态显示“启用菜单”，开启状态显示“关闭”。
- 配置页移除了原“启用右键菜单”卡片与“高级维护”外层折叠容器，改为 4 个一级 `details` 卡片，顺序为“右键菜单状态 / 迁移与清理 / Windows 11 经典菜单开关 / 运行与安装策略”，默认均不展开。
- “迁移与清理”内部的“菜单扫描结果”二级折叠、各按钮回调与状态展示逻辑保持不变；“运行与安装策略”中的终端运行器、uv 安装源和安装超时设置仅调整层级与位置，不改状态字段或保存逻辑。
- `src/components/UsageGuideDialog.tsx` 将“高级维护”旧引用更新为“配置”页中的“右键菜单状态”区域。
- 删除已无引用的 `src/components/ToggleRow.tsx`，并通过 `npm run build` 完成编译验证。

## 回顾
- 这类信息架构重排优先只改展示层级与文案，不碰状态源和业务回调，能明显降低 UI 调整带来的行为回归风险。
- 把全局开关移动到共享顶部栏时，需要同时补齐 hover / focus 提示和 `aria-label`，否则位置变化后可发现性会下降。

# 2026-03-06 版本 +0.0.1 与构建安装包（本次）
...
## 执行记录
- 执行 `npm run bump:patch`，版本由 `0.2.9` 升级至 `0.2.10`。
- `sync-version` 自动同步版本到：
- `src-tauri/Cargo.toml`
- `src-tauri/tauri.conf.json`
- `src-tauri/wix/main.wxs`
- 随后执行 `npm run sync-version:check`，校验通过，版本一致：`0.2.10`。
- 执行 `npm run tauri -- build` 成功；构建过程中前端 `npm run build` 也通过。
- 产物已生成：
- `src-tauri/target/release/bundle/msi/ExecLink_0.2.10_x64_zh-CN.msi`（3493888 bytes，2026-03-06 23:23:40）
- `src-tauri/target/release/bundle/nsis/ExecLink_0.2.10_x64-setup.exe`（2323856 bytes，2026-03-06 23:23:48）

## 回顾
- 按项目既定流程使用 `bump:patch -> sync-version:check -> tauri build`，可以稳定保持 `package.json` 与 Tauri 清单版本一致，避免打包阶段被版本漂移阻断。
- 在已有未提交工作区上做版本提升时，应只增量改动版本清单和任务记录，不回退现有业务修改。

# 2026-03-06 更新使用说明向导与 README

## 计划清单
- [x] 将 `?` 按钮弹窗从简短步骤升级为分区说明 + 可滚动的详细使用说明向导。
- [x] 根据当前页面与实际按钮文案，重写弹窗中的顶部入口、CLI 页、配置页、生效与排障说明。
- [x] 更新 `README.md` 的核心能力、使用说明与恢复清理段落，并修正与现状不符的旧表述。
- [x] 运行 `npm run build` 并搜索关键旧词，验证说明文案与当前 UI 一致。
- [x] 回填执行记录与回顾。

## 执行记录
- `src/components/UsageGuideDialog.tsx` 将原先 5 条简短步骤改为 4 个固定分区：顶部入口与首次操作、CLI 页怎么用、配置页怎么用、生效方式与排障。
- 弹窗容器调整为带最大高度的纵向布局，正文区域启用内部滚动，保持现有宽度风格，同时继续支持遮罩点击、`Esc` 和底部“我知道了”关闭。
- 弹窗文案已按当前真实 UI 同步：`刷新 CLI 检测`、动态前置依赖按钮、`应用配置`、`快速安装向导`、`仅执行安装`、`右键菜单状态`、`迁移与清理`、`Windows 11 经典菜单开关`、`运行与安装策略`、`Explorer 兜底刷新`。
- `README.md` 更新了“核心能力”“使用说明（按页面）”“恢复与清理”段落，补充 CLI 页与配置页的真实功能，并移除了与当前界面不符的“高级维护”等旧表述。
- 执行 `npm run build` 通过。
- 执行 `rg -n "高级维护|菜单页|菜单 tab|使用向导（快速上手）" README.md src` 未返回结果，说明 README 和前端源码中的关键旧词已清理。
- 静态核对通过：
- `UsageGuideDialog.tsx` 已包含 4 个分区标题。
- `README.md` 已包含新的“使用说明（按页面）”与更新后的“恢复与清理”标题。
- 本轮未实际启动桌面窗口做手动点击与滚动验证；已完成编译与静态内容核对。

## 回顾
- 说明类改动也要以当前真实按钮文案和页面结构为准，否则文档很快会和 UI 脱节；像“动态前置依赖按钮”这类入口不应写死成单一按钮名。
- 对详细帮助弹窗，优先使用分区 + 内部滚动，而不是无限堆长列表；这样既能补足说明深度，也不会破坏当前窗口尺寸和移动端可用性。

# 2026-03-06 配置页 Win11 容器重复标题修正

## 计划清单
- [x] 删除 `Windows 11 经典菜单开关` 折叠容器内部重复出现的同名标题。
- [x] 保留折叠容器 `summary` 标题，仅调整容器正文排版。
- [x] 运行 `npm run build` 验证前端编译通过。
- [x] 回填执行记录与回顾。

## 执行记录
- `src/pages/Home.tsx` 中的 `Windows 11 经典菜单开关` 折叠容器保留外层 `summary` 标题，删除正文中的重复 `h2`。
- 原标题下的说明文字改为直接参与头部布局，与右侧状态 chip 并排显示，避免同一容器内出现两次相同标题。
- 执行 `npm run build` 通过。

## 回顾
- 折叠容器的 `summary` 已承担标题语义时，正文不应再重复渲染同名 `h1/h2`；否则会造成视觉冗余，也容易被用户立即感知为回归。

# 2026-03-06 构建安装包（当前工作区）

## 计划清单
- [x] 核对当前版本与工作区状态，确认直接在现有修改上构建。
- [x] 执行 `npm run tauri -- build`。
- [x] 核验 MSI / NSIS 产物路径、时间戳与版本号。
- [x] 回填执行记录与回顾。

## 执行记录
- 构建前核对当前版本为 `0.2.10`，且工作区包含未提交修改；本次直接在当前工作区上执行打包，未回退现有改动。
- 执行 `npm run tauri -- build` 成功；命令链路中的 `npm run sync-version:check` 通过，前端 `npm run build` 也通过。
- 产物已生成：
- `src-tauri/target/release/bundle/msi/ExecLink_0.2.10_x64_zh-CN.msi`（3493888 bytes，2026-03-06 23:38:32）
- `src-tauri/target/release/bundle/nsis/ExecLink_0.2.10_x64-setup.exe`（2323198 bytes，2026-03-06 23:38:37）

## 回顾
- 当前项目的打包入口已经把版本一致性校验串进 `npm run tauri -- build`，因此只要 `sync-version:check` 通过，就可以直接复用这一入口完成构建。
- 在工作区带着功能改动时重复打包是安全的，但应在记录中明确说明“本次产物基于当前未提交工作区”，避免后续把它误认为干净基线构建结果。
